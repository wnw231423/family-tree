...
导入和导出的族谱 CSV 不包含用户、创建者或协作者信息。CSV 中通过文件内的 `legacy_id` 标识成员，并使用 `parent_legacy_id`、`child_legacy_id`、`husband_legacy_id`、`wife_legacy_id` 表示关系。用户登录后在 Dashboard 上传 CSV，系统创建新族谱并将当前用户写入 `creator_id`；导出时从族谱详情页下载同一格式的 CSV 文件。
