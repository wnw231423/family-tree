...
Dashboard 显示当前用户可访问族谱的总人数、男性人数、女性人数。族谱管理支持新增、编辑、删除、邀请协作者、CSV 导入和 CSV 导出。成员管理支持新增、编辑、删除、按姓名模糊查找、查看父母、子女和配偶，并可通过成员 ID 维护亲子关系和婚姻关系。
...
数据生成脚本位于 `data/` 目录。CSV 导入和导出由 Web 页面提供。

| 文件 | 作用 |
| --- | --- |
| data/generate.py | 生成以单个族谱为单位的纯数据 CSV |
...
导入和导出的族谱 CSV 不包含用户、创建者或协作者信息。CSV 中通过文件内的 `legacy_id` 标识成员，并使用 `parent_legacy_id`、`child_legacy_id`、`husband_legacy_id`、`wife_legacy_id` 表示关系。用户登录后在 Dashboard 上传一个或多个 CSV 文件，系统为每个文件创建一个新族谱并将当前用户写入 `creator_id`；导出时从族谱详情页下载同一格式的 CSV 文件。
...
| 数据生成 | Faker，本地安装于 `requirements-data.txt` |
| 部署 | Docker Compose |
...
前端采用 Bootstrap CDN 与 Jinja2 模板，能够满足登录、CRUD、树形预览和查询结果展示需求。数据生成采用 Python 脚本输出 CSV，导入和导出通过系统页面完成。项目通过 Docker Compose 启动 Flask 应用容器和 PostgreSQL 数据库容器。数据库容器首次创建数据卷时执行 `sql/schema.sql` 完成初始化；若数据卷已存在，需要手动导入 schema 或重建数据库数据卷。
